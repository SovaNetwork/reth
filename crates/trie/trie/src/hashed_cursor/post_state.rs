...
impl<CF: HashedCursorFactory> HashedCursorFactory for HashedPostStateCursorFactory<CF> {
    ...
    fn hashed_storage_cursor(
        ...
        Ok(HashedPostStateStorageCursor::new(
            cursor,
            self.post_state.storages.get(&hashed_address).cloned(),
        ))
    }
}
    ...
    C: HashedStorageCursor<Value = U256>,
{
    ...
    pub fn new(cursor: C, post_state_storage: Option<HashedStorageSorted>) -> Self {
        let post_state_cursor = post_state_storage
            .clone()
            .map(|s| ForwardInMemoryCursor::new(Arc::new(s.non_zero_valued_slots)));
